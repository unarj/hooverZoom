...
panel.port.on('click', function() { pageMod. });
...
panel.build = function(img, x, y) {
	if(img == this.curUrl) {
		console.log("src: "+this.curSrc+"  url: "+this.curUrl);
...
pageWorker.port.on('current', function(src, url) { panel.curSrc = src; panel.curUrl = url }),
