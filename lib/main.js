...
panel.port.on('click', function() { pageMod.port.emit('load', panel.curSrc); console.log(panel.curSrc) });
...
pageWorker.port.on('current', function(src, url) { panel.curSrc = src; panel.curUrl = url; console(panel.curSrc+" "+panel.curUrl) }),
pageWorker.port.on('hide', function() { if(panel.isShowing) { panel.hide() } });
pageWorker.port.on('image', function(img, x, y) { panel.setOnAlbum(true); panel.build(img, x, y) });
...
function setPrefs() {
	...
	pageMod = pageMods.PageMod({
		...
		onAttach: function(worker) {
			...
			worker.port.on('current', function(src, url) { panel.curSrc = src; panel.curUrl = url }),
