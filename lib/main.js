...
panel.build = function(img, x, y, txt) {
	...
	this.resize(x, y);
	if(this.isShowing) { this.position = {} } else { this.show() }
...
pageWorker.port.on('current', function(src) { panel.curSrc = src }),
...
function setPrefs() {
	...
	pageMod = pageMods.PageMod({
		...
		onAttach: function(worker) {
			...
			worker.port.on('current', function(src) { panel.curSrc = src }),
