...
panel.port.on('click', function(pass) { if(pass){pageMod.port.emit('click')}; panel.hide()});
...
function setPrefs() {
	...
	pageWorker.port.on('image', function(img, x, y, txt) { if(pageWorker.contentURL != 'about:blank') { panel.showImage(img, x, y, txt) } });
	pageWorker.port.on('video', function(vid, x, y, txt) { if(pageWorker.contentURL != 'about:blank') { panel.showVideo(vid, x, y, txt) } });
}
setPrefs();