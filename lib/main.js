...
function setPrefs() {
	if(prefs.blacklist) {
		var inc = ['*']; var exc = prefs.domainlist.split(',');
	} else {
		var inc = prefs.domainlist.split(','); var exc = [];
	}
	pageMod = pageModLib.PageMod({
		...
		onAttach:function(worker) {
			worker.port.on('load', function(url) { pageWorker.contentURL = url }),
			worker.port.on('visit', function(url) { history.updatePlaces({ uri:uri(url, 0), visits:[{transitionType:1, visitDate:Date.now()*1000}] }) })
		}
	});
	pageWorker = pageWorkerLib.Page({
		contentURL:'about:blank',
		contentScriptFile:"./page-worker.js",
		contentScriptWhen:'ready'
	});
	pageWorker.port.on('image', function(url, image){ pageMod.port.emit('image', url, image) });
	pageWorker.port.on('video', function(url, video){ pageMod.port.emit('video', url, video) });
}
setPrefs();
