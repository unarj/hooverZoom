...
panel.port.on('click', function() { pageMod.port.emit('click') });
...
function setPrefs() {
	...
	pageWorker.port.on('load', function(url) { panel.curUrl = url; pageWorker.contentURL = url });
	pageWorker.port.on('image', function(img, url, x, y, txt) { panel.showImage(img, url, x, y, txt) });
	pageWorker.port.on('video', function(vid, url, x, y, txt) { panel.showVideo(vid, url, x, y, txt) });
}
setPrefs();
