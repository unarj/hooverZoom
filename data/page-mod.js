//if(unsafeWindow.self == unsafeWindow.top) {
	...
	self.port.on('onAlbum', function(state) {
		if(hzOnAlbum != state) {
			hzOnAlbum = state;
			if(hzOnAlbum) {
				window.top.addEventListener('DOMMouseScroll', hzWheel, false);
				console.log("added listener")
			} else {
				window.top.removeEventListener('DOMMouseScroll', hzWheel, false);
				console.log("removed listener")
			}
		}
	});

	self.port.on('load', function(url) { console.log(window.location.href); window.location.href = url; console.log("pageMod loading: "+url) });
//}